...
Nuvaryn is an iPhone app that gives an [OpenClaw](https://openclaw.ai) agent a real-time 3D presence:
...
**No Nuvaryn servers. Your conversations never leave your devices and the providers you choose.**

| | |
|---|---|
| 📲 **App** | Nuvaryn on the App Store (coming soon) |
...
| 🧩 **Host tooling** | [`host/setup-host.sh`](host/setup-host.sh) (one-shot configurator) · [`host/nuvaryn-bridge.py`](host/nuvaryn-bridge.py) |
...
```
 iPhone (Nuvaryn app)                    Mac mini (always on)                     Cloud / local
 ┌───────────────────┐   Tailscale HTTPS  ┌─────────────────────────────┐
 │ 3D stage (three.js)│◀────── wss ──────▶│ OpenClaw gateway :18789      │──▶ OpenAI / Anthropic /
 │ voice + lip-sync   │◀──── /bridge ────▶│ Nuvaryn bridge  :18790       │    Google / OpenRouter
...
Everything you need to **host** Nuvaryn: setup script, bridge service, guides, privacy policy, and the character assets.
